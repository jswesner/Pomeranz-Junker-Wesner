...
# 
# saveRDS(b.mod, file = "final_models/b.mod.RDS")
# saveRDS(biomass.mod, file = "final_models/biomass.mod.RDS")
...
post_prior_predict <- draws_dm_withx %>% 
  left_join(site_offsets) %>% 
  left_join(year_offsets) %>% 
  mutate(fitted = exp(intercept + b*mat.c), #solve equation at each iteration. log-link is indicated by the exp().
         pred = case_when(
           model == "prior" ~
             exp(intercept + site_offset + year_offset + b*mat.c),
                          TRUE ~ fitted)) #same but with random offsets added.
...
plot_biomass_postprior <- post_prior_predict %>% 
  ...
  ggplot(aes(x = mat.c,
             y = pred)) + 
  geom_line(aes(group = interaction(model,b)),
            alpha = 0.1) +
  facet_wrap(~model) +
  guides(color = F) +
...
prior_sens_plot <- all_b_sens %>% 
  mutate(name = case_when(name == "b_mat_c" ~ "\u03b2", 
                          name == "b_intercept" ~ "\u03b1")) %>% 
  ggplot(aes(x = value,
             color = model,
             y = ..scaled..)) +
